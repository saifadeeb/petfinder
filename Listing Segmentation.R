...
### Kmeans without target variable ###
...
wssplot(trainsub4) #7 actually shows no significant difference from 8 so we'll check 6 and 7
trainsub4.dist = dist(trainsub4)

#######################################################
######  k means with reduced data with 7 clusters######
#######################################################

clusterresults <- kmeans(trainsub4,7)
clusterresults$size #3144  365 3544 1026 2810 2591 1513
rsquare <- clusterresults$betweenss/clusterresults$totss
rsquare #0.6905553 <- better than previous
...
sk2   <- silhouette(clusterresults$cluster, trainsub4.dist) #0 on the wall, 1 is better - quality is poor
str(sk2)
#jpeg("./Images/kmeans sil.jpg")
plot(sk2, border=NA) #avg silhouette = 0.18
#dev.off()
...
colnames(combdata)[79] <- "cluster"
head(combdata)

clusters <- subset(combdata, select=c('PetID','cluster'))
write.csv(clusters, file="Pet_train_clusters.csv")


ggplot(combdata, aes(x=factor(cluster), y=AdoptionSpeed)) + 
...
library(plyr)
agg <- count(combdata, c('cluster','AdoptionSpeed'))
agg[which(agg$AdoptionSpeed==0 | agg$AdoptionSpeed==1 | agg$AdoptionSpeed==2),2] <- "Fast"
agg[which(agg$AdoptionSpeed==3 | agg$AdoptionSpeed==4),2] <- "Slow"
agg2 <- count(agg, c('cluster','AdoptionSpeed'))

ggplot(agg2, aes(x=factor(cluster), y=freq)) + 
  geom_bar(aes(fill=factor(AdoptionSpeed)), stat="identity", position="dodge") +
  labs(title="Quantity Adoption Speed per Cluster", x="Cluster", y="Count") +
  theme(plot.title=element_text(lineheight=0.8, face="bold", hjust=0.5))

test <- read.csv("test/test.csv", stringsAsFactors = FALSE)


#Transform Variables
test$Age_yr <- floor(test$Age/12)
#Breed1 - Focused on Breed1 - Breed2 showed 71% values were blank
#Domestic Short Haired Cat
test$cat_short <- ifelse(test$Breed1==266 & test$Type==2, 1, 0)
#Domestic Medium Haired Cat
test$cat_med <- ifelse(test$Breed1==265 & test$Type==2, 1, 0)
#Tabby Cat
test$cat_tabby <- ifelse(test$Breed1==299 & test$Type==2, 1, 0)
#Domestic Long Haired Cat
test$cat_long <- ifelse(test$Breed1==264 & test$Type==2, 1, 0)
#Specific Cat breed given
test$cat_breed <- ifelse(test$Type==2 & test$cat_short==0 & test$cat_med==0 &
                            test$cat_tabby==0 & test$cat_long==0, 1, 0)
#Mixed Dog
test$dog_mixed <- ifelse(test$Breed1==307 & test$Type==1, 1, 0)
#Specific Dog breed given
test$dog_breed <- ifelse(test$Type==1 & test$Breed1!=307, 1, 0)
#Dog
test$dog <- ifelse(test$Type==1, 1, 0)
#Gender
test$male <- ifelse(test$Gender==1, 1, 0)
test$female <- ifelse(test$Gender==2, 1, 0) #0 would be mixed gender
#Color1 - #focused on Color1 and Color2 - Color3 showed 71% blanks
test$black1 <- ifelse(test$Color1==1, 1, 0)
test$brown1 <- ifelse(test$Color1==2, 1, 0)
test$golden1 <- ifelse(test$Color1==3, 1, 0)
test$yellow1 <- ifelse(test$Color1==4, 1, 0)
test$cream1 <- ifelse(test$Color1==5, 1, 0)
test$gray1 <- ifelse(test$Color1==6, 1, 0)
#Color2 - Black was not a given color so we will use 2-7 because blanks are an option
test$brown2 <- ifelse(test$Color2==2, 1, 0)
test$golden2 <- ifelse(test$Color2==3, 1, 0)
test$yellow2 <- ifelse(test$Color2==4, 1, 0)
test$cream2 <- ifelse(test$Color2==5, 1, 0)
test$gray2 <- ifelse(test$Color2==6, 1, 0)
test$white2 <- ifelse(test$Color2==7, 1, 0)
#Maturity Size
test$size_small <- ifelse(test$MaturitySize==1, 1, 0)
test$size_medium <- ifelse(test$MaturitySize==2, 1, 0)
test$size_large <- ifelse(test$MaturitySize==3, 1, 0)
#FurLength
test$fur_short <- ifelse(test$FurLength==1, 1, 0)
test$fur_med <- ifelse(test$FurLength==2, 1, 0)
#Vaccinated
test$vac_yes <- ifelse(test$Vaccinated==1, 1, 0)
test$vac_no <- ifelse(test$Vaccinated==2, 1, 0)
#Dewormed
test$worm_yes <- ifelse(test$Dewormed==1, 1, 0)
test$worm_no <- ifelse(test$Dewormed==2, 1, 0)
#Sterilized
test$ster_yes <- ifelse(test$Sterilized==1, 1, 0)
test$ster_no <- ifelse(test$Sterilized==2, 1, 0)
#Health
test$health_healthy <- ifelse(test$Health==1, 1, 0)
test$health_minor <- ifelse(test$Health==2, 1, 0)
#Fee - no fee versus fee
test$fee_free <- ifelse(test$Fee==0, 1, 0)
#State
test$state_41324 <- ifelse(test$State==41324, 1, 0)
test$state_41325 <- ifelse(test$State==41325, 1, 0)
test$state_41326 <- ifelse(test$State==41326, 1, 0)
test$state_41327 <- ifelse(test$State==41327, 1, 0)
test$state_41330 <- ifelse(test$State==41330, 1, 0)
test$state_41332 <- ifelse(test$State==41332, 1, 0)
test$state_41335 <- ifelse(test$State==41335, 1, 0)
test$state_41336 <- ifelse(test$State==41336, 1, 0)
test$state_41342 <- ifelse(test$State==41342, 1, 0)
test$state_41345 <- ifelse(test$State==41345, 1, 0)
test$state_41361 <- ifelse(test$State==41361, 1, 0)
test$state_41367 <- ifelse(test$State==41367, 1, 0)
test$state_41401 <- ifelse(test$State==41401, 1, 0)

#combine like colors
test$yellow_gp1 <- test$yellow1 + test$golden1
test$white_gp1 <- test$cream1 + ifelse(test$Color1==7, 1, 0)
test$yellow_gp2 <- test$yellow2 + test$golden2
test$white_gp2 <- test$white2 + test$cream2

### Creating subset ###
testsub <- subset(test, select=
                    c("Age_yr","male","female",
                      "black1","brown1","yellow_gp1","white_gp1","size_small","size_medium",
                      "size_large","vac_yes","vac_no","worm_yes","worm_no","ster_yes",
                      "ster_no","health_healthy","health_minor","fee_free",
...
require(clue)
predict<-cl_predict(clusterresults, newdata= testsub, type=c("class_ids"))
predict<-paste(predict,sep="")

newdf <- as.data.frame(predict)
combdata <- cbind(test,newdf)

head(combdata)

colnames(combdata)[78] <- "cluster"
head(combdata)

clusters <- subset(combdata, select=c('PetID','cluster'))
write.csv(clusters, file="Pet_test_clusters.csv")
